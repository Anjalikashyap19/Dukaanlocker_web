...
export default function Support() {
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-7xl px-4">
          <div className="reveal-stagger grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {quickLinks.map((link) => (
              <button
                key={link.label}
                onClick={() => setOpenIndex(faqs.findIndex((f) => f.category === link.category))}
                className="group flex items-center gap-3 rounded-xl bg-card p-4 shadow-soft ring-1 ring-border card-hover cursor-pointer text-left"
              >
                <div className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-brand-soft text-brand ring-1 ring-brand/10 dark:bg-brand-soft/10 transition-all duration-300 group-hover:scale-110 group-hover:bg-brand group-hover:text-white group-hover:shadow-glow">
                  {link.icon}
                </div>
      ...
      {/* FAQ */}
      <section className="relative py-16 border-t border-border" ref={faqRef}>
        <div className="mx-auto max-w-3xl px-4">
          <div className="reveal text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-brand/20 bg-brand-soft/50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-brand backdrop-blur dark:bg-brand-soft/10">
              <HelpCircle className="h-3 w-3" aria-hidden="true" /> FAQs
            </div>
            ...
            {filteredFaqs.length === 0 ? (
              ...
              filteredFaqs.map((faq, idx) => {
                ...
                return (
                  <div
                    key={faq.q}
                    className={`overflow-hidden rounded-2xl bg-card shadow-soft ring-1 transition-all duration-300 ${isOpen ? 'ring-brand/30 shadow-card' : 'ring-border hover:ring-brand/20 hover:shadow-card'}`}
                  >
                    <button
                    ...
                    <div className={`grid transition-all duration-400 ease-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                      <div className="overflow-hidden">
                        <p className="px-5 pb-5 text-sm text-ink-soft leading-relaxed border-t border-border/50 pt-3">{faq.a}</p>
                      </div>
                    </div>
                  </div>
                );
      ...
      {/* Contact support */}
      <section className="relative py-20 border-t border-border bg-gradient-to-b from-transparent to-brand-soft/20 dark:to-brand-soft/5" ref={contactRef}>
        <div className="mx-auto max-w-7xl px-4">
      ...
      <section className="relative py-24 border-t border-border" ref={resourceRef}>
        <div className="mx-auto max-w-7xl px-4">
          <div className="reveal mx-auto max-w-2xl text-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-brand/20 bg-brand-soft/50 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-brand backdrop-blur dark:bg-brand-soft/10">
